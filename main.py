...
"""
def go2sortingdir(os_name, config: Config) -> str:
    
    #print(os_name)
    path = generate_full_destination_path("%s%s%s" % (get_home_directory(), config.file_path_seperator, config.sorting_directory), config)
...
def generate_full_destination_path(path_string, config: Config):
    if path_string[0] in ("\\", "/", "~"):
        ...
        return "%s%s%s" % (get_home_directory(), config.file_path_seperator, path_string)
...
if __name__ == '__main__':
    ...
    path = go2sortingdir(os_name, config)
    #print(get_file_list())
